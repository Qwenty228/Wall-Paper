...
# credits: https://www.shadertoy.com/view/XcXXzS
# Full Spectrum Cyber by https://www.shadertoy.com/user/Kali
...
float de(vec3 p) {
    ...
    vec3 pp=p-hpos;
    pp=lookat(point)*pp;
    pp.y-=abs(sin(time))*3.+(fparam-(2.-fparam));
    pp.yz*=rot(-time);
...
vec3 march(vec3 from, vec3 dir) {
    ...
    if (d<.001) {
        ...
        col+=step(.3,hp)*step(.9,fract(pr.z*.05+time*.5+hp*.1))*.7;
        col+=step(.3,hexpos)*step(.9,fract(zz*.05+time+hexpos*.1))*.3;
    }
    col+=g*.03;
	col.rb*=rot(odir.y*.5);
	return col;
...
void main()
{
    ...
    point.xz*=rot(sin(time)*.2);
    dir=lookat(dd)*dir;
    vec3 col = march(from, dir);
	col*=vec3(1.,.9,.8);
    f_color = vec4(col + texture(tex, uvs).rgb,1.0);
}
'''
