    ...
    def __init__(self, controller,  **kwargs):
        ...
        for i, vid in enumerate(os.listdir(path), 1):
            ...
            button.configure(command=lambda btn=button,
                             p=fullpath: controller.button_event(btn, p))
        self.n_button = i + 1
...
class Page2(customtkinter.CTkFrame):
    def __init__(self, controller,  **kwargs):
        super().__init__(**kwargs)
        self.controller = controller
        for i, template in enumerate(glob(r'data\template\*.py')):
            if 'video' in template: continue
            template = template.replace('\\', '.')[:-3]
            button = customtkinter.CTkButton(master=self,
                                            text=template.split('.')[-1], text_color='white', corner_radius=5, border_width=3,
                                            compound="top", fg_color="gray25", hover_color="gray30",  border_color='gray10')
            button.grid(row=(i//4), column=(i % 4), sticky='ew')
            button.configure(
                command=lambda btn=button, p=template: controller.button_event(btn, p))
