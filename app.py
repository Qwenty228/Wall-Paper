...
class App(customtkinter.CTk, AsyncTk):
    ...
    def get_first_frame(self, video):
        if (thumbnail := self._thumbnail.get(video)) is None:
            button_size = (200, 150)
            array = next(iio.imiter(video, plugin='pyav'))
            img = Image.fromarray(array)
            ratio = min([l/s for s, l in zip(array.shape[:2], button_size)])
            img = img.resize([int(i*ratio)
                             for i in array.shape[:2][::-1]], Image.Resampling.LANCZOS)
    ...
    def __init__(self, loop, update_interval=1/20):
        ...
        customtkinter.CTkLabel(master=self.frame_right, text="select background:").grid(
            row=0, column=0, pady=0, padx=10)

        switch_1 = customtkinter.CTkSwitch(
            master=self.frame_right, text='Live Wallpaper ON', variable=tkinter.IntVar(value=1))
    ...
    def set_up_frame(self):
        ...
        for i, page in enumerate(classes):
            ...
            self._pages[i] = frame
            frame.grid(row=1, column=0, columnspan=2, rowspan=4,
                       pady=0, padx=20, sticky="nsew")
    ...
    def get_video(self, button: customtkinter.CTkButton):
        ...
        video = filedialog.askopenfilename(title='Select Video or GIF', filetypes=[
                                           ('files', VID_FILE_TYPES)])
        if video:
    ...
    def button_event(self, button: customtkinter.CTkButton = None, path=None):
        if path:
            ...
            self.task = self.loop.create_task(
                self.wallpaper.draw(self.radio_var.get(), path))
...
if __name__ == "__main__":
    gui = App(asyncio.new_event_loop())
    gui.initialize()
    gui.mainloop()
